...
const LocationSelect: React.FC<LocationSelectProps> = ({ value, onChange }) => {
  // useLocation now also provides the coordinates
  const { selectedLocation, handleChange, coordinates } = useLocation(
    onChange,
    value
  );

  // You can use useEffect here if you want to perform any action when coordinates change
  React.useEffect(() => {
    if (coordinates.lat && coordinates.lng) {
      console.log("Coordinates: ", coordinates);
      // Perform any action you need with the new coordinates
    }
  }, [coordinates]);
